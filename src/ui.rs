...
pub fn render(app: &mut App, frame: &mut Frame) {
        ...
        let &(cur_line, cur_offset) = app.line_index.get(app.sample_start_index + app.cur_char).unwrap();
        let &(end_line, end_offset) = app.line_index.get(app.sample_start_index + app.sample_len).unwrap();
        let mut lines: Vec<String> = app.book_lines.clone();
        let num_rows = frame.size().height as usize - 2;
        let rows_to_center = num_rows / 2 - 2;

        if app.following_typing {app.display_line = cur_line}
        
        app.display_line = usize::min(app.display_line, lines.len());

        let first_row = usize::checked_sub(rows_to_center,app.display_line)
            .unwrap_or(0);

        let num_skipped_lines = usize::checked_sub(app.display_line, rows_to_center)
            .unwrap_or(0);
